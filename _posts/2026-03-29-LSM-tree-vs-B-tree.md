...
## LSM-tree
LSM(log-structured and merge)-tree is an efficient database engine which is suitable for high write situation. When user write data, it's stored in memory at first. This can be very efficient and these data can be sorted in a short time. The data structure used in memory is often called memtable. After the data reach some threshhold, these data will be appended to disk. The process act like we appending data to log file. These data is appended at the end of one segment sequencially. Sence we don't need to maintain any fixed structure like B-tree, this process could be finished very quickly. These segment is called SSTable.
